...
districts = gpd.read_file(
    os.path.join(project_root, "data", "visualisation", "districts_paris.geojson")
)


def plot_combined_output(
    ...
    if scale_type == "discrete":
        ...
        colors = plt.cm.RdYlGn_r(
            np.linspace(0, 1, n_categories)
        )  # Using _r to invert the colormap
    ...
    small_lines.plot(
        ...
        norm=norm,
        label=(
            "Street network" if with_legend else None
        ),  # Only add label if legend is wanted
        zorder=1,
    )
    large_lines.plot(
        column=column_to_plot,
    ...
    if plot_policy_roads:
        ...
        edges_with_capacity_reduction = gdf[
            np.abs(gdf["capacity_reduction_rounded"]) > tolerance
        ]
        edges_with_capacity_reduction.plot(
            ...
            norm=norm,
            label=(
                "Capacity was decreased on these roads" if with_legend else None
            ),  # Only add label if legend is wanted
            zorder=3,
        )

    relevant_area_to_plot = get_relevant_area_to_plot(
        known_districts, buffer, districts_of_interest, gdf
    )
    if plot_contour_lines:
        if isinstance(relevant_area_to_plot, set):
            ...
            relevant_area_to_plot.plot(
                ax=ax, edgecolor="black", linewidth=2, facecolor="None", zorder=2
            )
    ...
    if is_absolute:
        ...
        cbar.set_label(
            "Car volume: Difference to base case (%)", fontname=font, fontsize=15
        )
    if save_it:
        p = "predicted" if is_predicted else "actual"
        identifier = (
            "n_" + str(number_to_plot) if number_to_plot is not None else zone_to_plot
        )
...
def plotting(
    ...
    if with_legend:
        ...
        custom_lines = [
            Line2D([0], [0], color="grey", lw=4, label="Street network")
        ]  # Add more lines for other labels as needed
        ...
        if plot_contour_lines:
            custom_lines.append(
                Line2D(
                    ...
                    color="black",
                    lw=2,
                    label="Capacity was decreased in this section",
                )
            )
        ...
        if plot_policy_roads:
            custom_lines.append(
                Line2D(
                    ...
                    color="black",
                    lw=2,
                    label="Capacity was decreased on these roads",
                )
            )
    ...
    if with_legend:
        ...
        cax = fig.add_axes(
            [0.92, 0.15, 0.02, 0.7]
        )  # Adjusted colorbar position for no-legend case
...
def plot_average_prediction_differences(
    gdf_inputs: list,
    font: str = "Times New Roman",
    save_it: bool = False,
    use_fixed_norm: bool = True,
            ...
            if use_absolute_value_of_difference:
                if loss_fct == "l1":
                    error = (
                        abs(
                            (
                                gdf["vol_car_change_predicted"]
                                - gdf["vol_car_change_actual"]
                            )
                            / (
                                abs(
                                    gdf["vol_car_change_actual"]
                    ...
                    error = (
                        ...
                        / (
                            abs(gdf["vol_car_change_actual"] + gdf["vol_base_case"])
                            + epsilon
                        )
                    ) * 100
            else:
                if loss_fct == "l1":
                    error = (
                        (gdf["vol_car_change_predicted"] - gdf["vol_car_change_actual"])
                        / (
                            gdf["vol_car_change_actual"]
                            + gdf["vol_base_case"]
                            + epsilon
    ...
    if disagreement_threshold is not None:
        ...
        mask = abs_mean > 1e-10
        base_gdf["coefficient_of_variation"] = float(
            "inf"
        )  # Default value for zero means
        base_gdf.loc[mask, "coefficient_of_variation"] = (
            base_gdf.loc[mask, "std_prediction_error"] / abs_mean[mask]
        )
        high_disagreement = (
            base_gdf["coefficient_of_variation"] > disagreement_threshold
        )
    ...
    if scale_type == "discrete":
        ...
        if use_fixed_norm:
            norm = (
                TwoSlopeNorm(vmin=-fixed_norm_max, vcenter=0, vmax=fixed_norm_max)
                if not use_absolute_value_of_difference
                else Normalize(vmin=0, vmax=fixed_norm_max)
            )
        else:
            norm = (
                TwoSlopeNorm(
                    vmin=base_gdf["mean_prediction_error"].min(),
                    vcenter=0,
                    vmax=base_gdf["mean_prediction_error"].max(),
                )
                if not use_absolute_value_of_difference
                else Normalize(vmin=0, vmax=base_gdf["mean_prediction_error"].max())
            )
    ...
    cbar.set_label(
        f"{error_type} Prediction Error ({units})",
    ...
    if save_it:
        error_type_str = (
            "average_absolute_value_of_difference"
            if use_absolute_value_of_difference
            else "average_signed_difference"
        )
        metric_str = "percent" if use_percentage else "abs_vehicles"
        plt.savefig(
            f"{result_path}/{error_type_str}_prediction_error_{metric_str}",
            bbox_inches="tight",
        )
...
def get_relevant_area_to_plot(known_districts, buffer, districts_of_interest, gdf):
    if known_districts:
        ...
        gdf["intersects_target_districts"] = gdf.apply(
            lambda row: target_districts.intersects(row.geometry).any(), axis=1
        )
...
def create_correlation_radar_plot_sort_by_r2(
    metrics_by_type,
    ...
    if selected_metrics is None:
        selected_metrics = [
            {"id": "r_squared", "label": "R²", "transform": lambda x: max(0, x)},
            # {
            #     'id': 'l1_ratio',
    ...
    max_ratios["mse"] = max(
        metrics_by_type[rt]["mse"] / metrics_by_type[rt]["naive_mse"]
        for rt in road_types
    )

    max_ratios["l1"] = max(
        metrics_by_type[rt]["l1"] / metrics_by_type[rt]["naive_l1"] for rt in road_types
    )
        ...
        for metric in selected_metrics:
            if "ratio" in metric["id"]:
                if metric["id"] == "mse_ratio":
                    ratio = (
                        filtered_metrics[road_type]["mse"]
                        / filtered_metrics[road_type]["naive_mse"]
                    )
                    values.append(metric["transform"](ratio, max_ratios["mse"]))
                elif metric["id"] == "l1_ratio":
                    ratio = (
                        filtered_metrics[road_type]["l1"]
                        / filtered_metrics[road_type]["naive_l1"]
                    )
            ...
            if "scaled" in metric["id"]:
                if metric["id"] == "l1_scaled":
                    val = (
                        filtered_metrics[road_type]["l1"]
                        / metrics_by_type[road_type]["mean_car_vol"]
                    )
                ...
                val = filtered_metrics[road_type].get(
                    "error_distribution", 68
                )  # Default to 68 if not found
                values.append(metric["transform"](val))
            else:
...
def create_error_vs_variability_scatterplots(
    metrics_by_type,
    manual_label_offsets=dict(),
    result_path=None,
    save_it=False,
    selected_types=None,
    colors=None,
    ...
    for i, rt in enumerate(selected_types):
        ...
        ax.annotate(
            rt,
            (variance_values[i], mse_values[i]),
            xytext=offset,
    ...
    handles = [
        Line2D(
            ...
            marker="o",
            linestyle="",
            color=colors[selected_types[argmin_obs]],
            label=f"{selected_types[argmin_obs]}: {min_obs:,}",
            markersize=(50) ** 0.5,
        ),
        Line2D(
            [0],
            ...
            marker="o",
            linestyle="",
            color=colors[selected_types[argmax_obs]],
            label=f"{selected_types[argmax_obs]}: {max_obs:,}",
            markersize=((max_obs / min_obs) * 50) ** 0.5,
        ),
    ]
    ...
    obs_legend = Legend(
        ...
        title=r"Dot Size $\propto$ #observations",
        loc="lower right",
        fontsize=15,
        title_fontsize=15,
        frameon=True,
    ...
    if save_it:
        plt.savefig(
            result_path + "error_vs_variability_scatterplot_with_line.png",
            bbox_inches="tight",
            dpi=300,
        )
