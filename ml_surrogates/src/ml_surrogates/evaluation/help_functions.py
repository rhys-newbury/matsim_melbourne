...
from scripts.data_processing.process_simulations_for_gnn import EdgeFeatures
from ml_surrogates.gnn.help_functions import compute_r2_torch_with_mean_targets
...
def data_to_geodataframe_with_og_values(
    data, original_gdf, predicted_values, inversed_x, use_all_features=False
):
    ...
    if use_all_features:
        edge_data = {
            ...
            "length": inversed_x[
                :, EdgeFeatures.LENGTH - 1
            ],  # -1 since we didn't use Highway, fix later
...
def create_test_object(links_base_case, test_data, stacked_edge_geometries_tensor):
    ...
    nodes = pd.concat(
        [links_base_case["from_node"], links_base_case["to_node"]]
    ).unique()
    ...
    linegraph_data.y = torch.tensor(
        edge_car_volume_difference, dtype=torch.float
    ).unsqueeze(1)
...
def get_road_type_indices(gdf, tolerance=1e-3):
    ...
    indices = {
        ...
        "Roads with Capacity Reduction": gdf[
            gdf["capacity_reduction_rounded"] < -tolerance
        ].index,
        "Roads with No Capacity Reduction": gdf[
            gdf["capacity_reduction_rounded"] >= -tolerance
        ].index,
        "P/S/T Roads with Capacity Reduction": gdf[
            (gdf["highway"].isin([1, 2, 3]))
            & (gdf["capacity_reduction_rounded"] < -tolerance)
        ].index,
        "P/S/T Roads with No Capacity Reduction": gdf[
            (gdf["highway"].isin([1, 2, 3]))
            & (gdf["capacity_reduction_rounded"] >= -tolerance)
        ].index,
        # Combined conditions
...
def validate_model_on_test_set(model, dataset, loss_func, device):
    ...
    with torch.inference_mode():
        if isinstance(dataset, list):
            for data in dataset:
                _input_node_features, targets = data.x.to(device), data.y.to(device)
            ...
            _input_node_features, targets = dataset.x.to(device), dataset.y.to(device)
    ...
    r_squared = compute_r2_torch_with_mean_targets(
        mean_targets=mean_targets, preds=all_preds, targets=all_targets
    )
