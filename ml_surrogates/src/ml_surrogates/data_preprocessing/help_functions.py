...
def create_policy_key(folder_name: Path):
    ...
    base_name = folder_name.name  # Get the base name of the file or folder
...
def read_output_links(folder: Path) -> pd.DataFrame | None:
    file_path = folder / "output_links.csv.gz"
    if file_path.exists():
        try:
...
def extend_geodataframe(
    ...
    gdf_to_extend: gpd.GeoDataFrame,
    column_to_extend: str,
    new_column_name: str,
):
    """
...
def calculate_avg_mode_stats(single_mode_stats_list: list[pd.DataFrame]):
    mode_stats_list = []
...
def encode_modes(gdf: gpd.GeoDataFrame) -> list[torch.Tensor]:
...
def read_eqasim_trips(folder: Path) -> pd.DataFrame | None:
    file_path = folder / "eqasim_trips.csv"
    if file_path.exists():
        try:
            df = pd.read_csv(file_path, delimiter=";")
            return df
...
def compute_target_tensor_only_edge_features(
    vol_base_case: NDArray, gdf: gpd.GeoDataFrame
) -> torch.Tensor:
...
def get_basic_edge_attributes(
    capacity_base_case: NDArray,
...
def prepare_gdf(df: pd.DataFrame, gdf_input: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
...
def get_link_geometries(
    links_gdf_input: gpd.GeoDataFrame,
) -> tuple[torch.Tensor, torch.Tensor, NDArray, NDArray]:
