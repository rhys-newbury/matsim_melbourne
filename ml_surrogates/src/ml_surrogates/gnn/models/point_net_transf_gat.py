...
from ml_surrogates.gnn.models.base_gnn import BaseGNN
...
class PointNetTransfGAT(BaseGNN):
    ...
    def define_gat_layers(self):
        ...
        for idx in range(len(self.gat_conv) - 1):
            ...
            layers.append(
                (
                    TransformerConv(
                        self.gat_conv[idx], int(self.gat_conv[idx + 1] / 4), heads=4
                    ),
                    "x, edge_index -> x",
                )
            )
    ...
    def create_point_net_layer(
        ...
        local_MLP_layers = []
        if is_first_layer:
            local_MLP_layers.append(
                nn.Linear(self.in_channels + offset_due_to_pos, self.pnc_local[0])
            )
        else:
            local_MLP_layers.append(
                nn.Linear(self.pnc_global[-1] + offset_due_to_pos, self.pnc_local[0])
            )
        ...
        for idx in range(len(self.pnc_local) - 1):
            local_MLP_layers.append(
                nn.Linear(self.pnc_local[idx], self.pnc_local[idx + 1])
            )
            local_MLP_layers.append(nn.ReLU())
            if self.use_dropout:
                local_MLP_layers.append(self.dropout_layer)
        ...
        for idx in range(len(self.pnc_global) - 1):
            global_MLP_layers.append(
                nn.Linear(self.pnc_global[idx], self.pnc_global[idx + 1])
            )
            global_MLP_layers.append(nn.ReLU())

        if is_last_layer:
            global_MLP_layers.append(
                nn.Linear(self.pnc_global[-1], gat_conv_starts_with_layer)
            )
        else:
            global_MLP_layers.append(
                nn.Linear(self.pnc_global[-1], self.pnc_global[-1])
            )
