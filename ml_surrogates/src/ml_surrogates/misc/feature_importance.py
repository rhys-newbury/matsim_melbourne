...
import ml_surrogates.gnn.gnn_io as gio
import ml_surrogates.gnn.models.point_net_transf_gat as garch
import ml_surrogates.help_functions as hf
...
with open(
    os.path.join(run_path, "data_created_during_training", "test_loader_params.json")
) as f:
    test_loader_params = json.load(f)

batch_size = test_loader_params["batch_size"]
test_set_normalized = torch.load(
    os.path.join(run_path, "data_created_during_training", "test_dl.pt")
)
test_loader = DataLoader(
    ...
    shuffle=True,
    num_workers=4,
    collate_fn=gio.collate_fn,
    worker_init_fn=hf.seed_worker,
)

# Load best model
model = garch.PointNetTransfGAT(
    in_channels=13,
    out_channels=1,
...
for i, batch in enumerate(tqdm(test_loader, desc="Explaining batches")):
    explanation = explainer(
        x=batch.x, edge_index=batch.edge_index, target=batch.y, data=batch
    )
...
feature_importances = np.mean(
    [exp.get("node_mask").sum(dim=0).cpu().detach().numpy() for exp in explanations],
    axis=0,
)
