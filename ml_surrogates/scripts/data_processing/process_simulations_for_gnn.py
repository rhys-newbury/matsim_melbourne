...
from ml_surrogates.data_preprocessing.help_functions import (
    ...
    read_eqasim_trips,
    extend_geodataframe,
    calculate_avg_mode_stats,
    get_basic_edge_attributes,
    get_link_geometries,
    encode_modes,
    prepare_gdf,
    compute_target_tensor_only_edge_features,
    EdgeFeatures,
)
...
def main():
    networks = sorted(
        ...
        for network in Path(path).iterdir()
        if network.is_dir() and network.name != ".DS_Store"
    )
